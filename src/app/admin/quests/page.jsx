      ...
      {editing && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(20,26,46,.35)', display: 'grid', placeItems: 'center', padding: 24, zIndex: 50 }} onClick={() => setEditing(null)}>
          <div className="card" style={{ width: 620, maxHeight: '88vh', overflow: 'auto' }} onClick={e => e.stopPropagation()}>
            ...
            <div className="grid2">
              <div className="field"><label>제목 · 나의 퀘스트 목록</label><input className="input" value={editing.title} onChange={e => setEditing({ ...editing, title: e.target.value })} placeholder="예: 고소리술 재료 찾기" /></div>
              <div className="field"><label>진행 화면 위 이름(핑크)</label><input className="input" value={editing.header_title || ''} onChange={e => setEditing({ ...editing, header_title: e.target.value })} placeholder="비우면 위 제목을 그대로 쓴다" /></div>
              <div className="field"><label>진행 화면 제목(검정)</label><input className="input" value={editing.screen_title || ''} onChange={e => setEditing({ ...editing, screen_title: e.target.value })} placeholder="비우면 장마다 적어둔 제목을 쓴다" /></div>
