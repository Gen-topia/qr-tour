...
export async function GET(request, { params }) {
  ...
  const [quest] = await q(
    'SELECT id, title, header_title, screen_title, quest_group, place, cover_image_url, clear_image_url, narration_video, clear_text, clear_audio_url FROM quests WHERE id=?', [id]
  );
  ...
  // 장소 안내를 보여주는 장({ "spot": true })이 있으면 코드 지도에서 이 미션의 장소를 찾아 함께 내려준다
  let spot = null;
  if (quest?.place && steps.some(s => s.config?.spot)) {
    const map = await readCodeMap();
    spot = (map.groups || []).flatMap(g => g.items || []).find(s => s.code === quest.place) || null;
  }

  // answer 컬럼은 미포함. dial의 목표 각도는 config에 있지만, 어차피 그만큼 돌려야 하므로 노출해도 무방하다.
  return ok({ quest: quest || null, steps, spot, locked: null });
}
