...
import { QUEST_REQUIRES, SEQUENTIAL_GROUPS, requireLabel } from '@/lib/questGroups';

// 지금 이 미션에 들어갈 수 있는지 서버에서 확인한다.
// 잠겨 있으면 안내 문구를, 열려 있으면 null을 돌려준다.
//   1) 선행 퀘스트 조건 — 예: 퀘스트3은 퀘스트1·퀘스트2를 모두 완수해야 열린다
  ...
  if (need?.length) {
    const [row] = await q(
      `SELECT COUNT(*) AS total, SUM(p.status = 'cleared') AS done
        ...
        WHERE qs.quest_group IN (?) AND qs.is_active = 1`,
      [userId, need]
    );
    const total = Number(row?.total || 0);
    const done = Number(row?.done || 0);
    if (total > 0 && done < total) {
      return `${requireLabel(need)}의 모든 퀘스트를 완수하세요.\n`
